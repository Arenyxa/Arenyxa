...
def run_features(run: dict[str,Any]) -> dict[str,float]:
    ...
    if pre and post:
        ...
        v=num(post.get('snapshot_duration_ms'))
        if v is not None:out['boundary_post_snapshot_duration_ms']=v
    return out
...
def main():
    ...
    reps=[a['transition'] for a in continuous if a.get('transition')]
    ...
    replicated_slow_fast=sum(bool(a['primary_p99_change_point'].get('meaningful') and a['primary_p99_change_point'].get('direction')=='slow_to_fast') for a in continuous)
    summary={'series_count':len(analyses),'continuous_count':len(continuous),'replicated_meaningful_slow_to_fast':replicated_slow_fast,
             'replication_axis':axis,'idle_transition':None if idle is None else idle.get('transition'),
             ...
             'initial_regime_candidate':('REPLICATED_SLOW_TO_FAST_TRANSITION' if replicated_slow_fast>=2 else 'INCONCLUSIVE_REPRODUCTION')}
    (d/'PHASE8_NUMERIC_SUMMARY.json').write_text(json.dumps(summary,indent=2,sort_keys=True),encoding='utf-8')
    for a in analyses:
        slim={k:v for k,v in a.items() if k!='runs'}
        (d/f"PHASE8_{a['series']}_ANALYSIS.json").write_text(json.dumps(slim,indent=2,sort_keys=True),encoding='utf-8')
    write_run_csv(d/'PHASE8_ALL_RUNS_FEATURES.csv',analyses)
    print(json.dumps({'analysis':'complete','series':len(analyses),'replicated_slow_fast':replicated_slow_fast,'axis':axis.get('most_stable_axis')},separators=(',',':')))
    return 0
