...
def compact_run(official: dict[str,Any], trace: dict[str,Any], *, name: str,
    ...
    ld=distribution(lease);ed=distribution(execute)
    ...
    return {
      ...
      'lease_p50':ld['p50'],'lease_p95':ld['p95'],'lease_p99':ld['p99'],'lease_p999':ld['p999'],
      'execute_p99_existing_phase6':ed['p99'],
...
def run_snapshot_calibration(root: Path, out: Path, *, name: str, port: int,
    ...
    container='p8-'+name.replace('_','-')
    dsn,meta=start_container(container,port,inst_dir)
    try:
        first=run_gate_off(dsn,root,inst_dir,name='first-run-retained',instance_meta=meta,warm_index=None)
        records=[];actions=[]
        for idx,mode in enumerate(CAL_PATTERN,1):
            ...
            s=run_gate_off(dsn,root,inst_dir,name=f'probe-{idx:02d}-{mode.lower()}',instance_meta=meta,warm_index=None)
...
def run_series(root: Path,out: Path,*,series_name: str,port: int,runs: int,
    ...
    container='p8-'+series_name.replace('_','-')
    dsn,meta=start_container(container,port,d)
    try:
        first=run_gate_off(dsn,root,d,name='first-run-retained',instance_meta=meta,warm_index=None)
        ...
        for i in range(1,runs+1):
            pre=current
            s=run_gate_off(dsn,root,d,name=f'warm-{i:03d}',instance_meta=meta,warm_index=i)
...
def main() -> int:
    ...
    env={'python':'.'.join(map(str,sys.version_info[:3])),'source_hashes':source_hashes,
         ...
         'phase6_minimal_off_baseline_reused':True,'calibration_pattern':list(CAL_PATTERN),
