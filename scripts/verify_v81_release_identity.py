...
DISPLAY_VERSION = "8.1"
DISTRIBUTION_VERSION = "8.1.0"
PACKAGE_VERSION = "8.1.0"
COMPAT_VERSION = "6.8.0"
FILE_VERSION = "8.1.0.0"
MODERN_INSTALLER = "Arenyxa_V8.1_Setup_x64"
...
def main() -> int:
    ...
    if match is None or match.group(1) != DISTRIBUTION_VERSION:
        raise RuntimeError("pyproject.toml does not carry the v8.1 stable distribution identity")

    _require("packaging/version_info.txt", "filevers=(8,1,0,0)")
    _require("packaging/version_info.txt", "prodvers=(8,1,0,0)")
    _require("packaging/version_info.txt", "FileDescription', 'Arenyxa V8.1'")
    _require("packaging/version_info.txt", f"FileVersion', '{FILE_VERSION}'")
    _require("packaging/version_info.txt", "ProductVersion', '8.1'")
    _require("packaging/installer.iss", '#define MyAppVersion "8.1"')
    ...
    _require("scripts/build_release_attestation.py", 'parser.add_argument("--version", default="8.1")')
    _require("RUN_ARENYXA.cmd", "title Arenyxa v8.1 Source Launcher")
    _require("README.md", "# Arenyxa V8.1")
    ...
    if matrix.get("product_release_version") != PACKAGE_VERSION:
    ...
    if docs_matrix.get("product_release_version") != PACKAGE_VERSION:
    ...
    print("Arenyxa v8.1 release identity gate: PASS")
