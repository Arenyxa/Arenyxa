...
RUNTIME_COMPATIBILITY_VERSION = "8.2"
DISPLAY_VERSION = "8.2.0"
DISTRIBUTION_VERSION = "8.2.0"
PACKAGE_VERSION = "8.2.0"
COMPAT_VERSION = "6.8.0"
FILE_VERSION = "8.2.0.0"
MODERN_INSTALLER = "Arenyxa_V8.2.0_Setup_x64"
LEGACY_INSTALLER = "Arenyxa_V8.2.0_Legacy_Win7_x64_Setup"
...
def main() -> int:
    ...
    if match is None or match.group(1) != DISTRIBUTION_VERSION:
        raise RuntimeError("pyproject.toml does not carry the v8.2.0 stable distribution identity")

    _require("packaging/version_info.txt", "filevers=(8,2,0,0)")
    _require("packaging/version_info.txt", "prodvers=(8,2,0,0)")
    _require("packaging/version_info.txt", "FileDescription', 'Arenyxa V8.2.0'")
    _require("packaging/version_info.txt", f"FileVersion', '{FILE_VERSION}'")
    _require("packaging/version_info.txt", "ProductVersion', '8.2.0'")
    _require("packaging/installer.iss", '#define MyAppVersion "8.2.0"')
    _require("packaging/installer.iss", f"OutputBaseFilename={MODERN_INSTALLER}")
    _require("packaging/installer_win7.iss", '#define MyAppVersion "8.2"')
    ...
    _require("scripts/build.ps1", "verify_v82_release_identity.py")
    _require("scripts/build.ps1", "$ProjectVersion = $Matches[1]")
    _require("scripts/build_release_attestation.py", 'parser.add_argument("--version", default="8.2.0")')
    _require("RUN_ARENYXA.cmd", "title Arenyxa v8.2.0 Source Launcher")
    _require("README.md", "# Arenyxa V8.2.0")
    _require("RELEASE_IDENTITY.json", '"display_version": "8.2"')
    _require("V8_2_RELEASE_IDENTITY.json", '"artifact_name": "Arenyxa_v8.2"')

    legacy_namespace = (ROOT / "legacy/win7/src/arenyxa/__init__.py").read_text(encoding="utf-8")
    for token in ('__version__ = "8.2"', '__package_version__ = "8.2.0"'):
    ...
    print("Arenyxa v8.2.0 release identity gate: PASS")
