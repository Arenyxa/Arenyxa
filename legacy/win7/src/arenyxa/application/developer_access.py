...
ROOT_WORKSTATION_BINDING_SCHEMA = "arenyxa.root-developer-workstation-binding/v1"
ROOT_WORKSTATION_PAYLOAD_SCHEMA = "arenyxa.root-developer-workstation-payload/v1"
...
class RootWorkstationBinding:
    ...
    def __init__(
        ...
        *,
        ...
        self.path = Path(data_root) / "developer" / "root_workstation.binding.json"
    ...
    def provision(self, bundle: Mapping[str, Any], *, at: datetime | None = None) -> RootWorkstationStatus:
        ...
        payload = {
            ...
            "provisioned_at": (datetime.now(UTC) if at is None else at).isoformat(),
        }
    ...
    def detect(self, *, at: datetime | None = None) -> RootWorkstationStatus:
        ...
        try:
            ...
            if wrapper.get("schema") != ROOT_WORKSTATION_BINDING_SCHEMA:
            ...
            } or payload.get("schema") != ROOT_WORKSTATION_PAYLOAD_SCHEMA:
            ...
            return RootWorkstationStatus(
                True, credential.owner_id, credential.root_key_id, credential.certificate_sha256,
                credential.owner_fingerprint, "VERIFIED",
            )
    ...
    def clear(self) -> None:
        try:
            ...
            raise _error("ROOT_WORKSTATION_CLEAR_FAILED", "Root Developer workstation binding could not be removed") from exc
...
class DeveloperAccessManager:
    ...
    def begin_root_owner_login(
        ...
        try:
            self.security.audit.emit(
                ...
                trust_domain=TrustDomain.DEVELOPER,
                correlation_id=challenge_id,
                reason="OWNER_CERTIFICATE_CHAIN_VALID",
            )
        except Exception:
            with self._lock:
                self._pending.pop(challenge_id, None)
        ...
        *,
        ...
        try:
            self.security.audit.emit(
                ...
                reason="OWNER_CERTIFICATE_AND_PRIVATE_KEY_PROOF_VALID",
            )
        except Exception:
            self._retire_session(session)
            raise
    ...
    def activate_root_workstation_session(self) -> Session | None:
        ...
        identity = self.security.state.create_identity(
            TrustDomain.DEVELOPER,
            principal_id=f"root-workstation:{binding.owner_id}:{binding.certificate_sha256[:16]}",
            display_name=binding.owner_id,
            kind="root_workstation",
        )
        identity.metadata.update({
        ...
        session = self.security.issue_session(
            identity.id,
            capabilities=sorted(DEVELOPER_CAPABILITIES) + ["platform.root"],
            ttl_seconds=OWNER_SESSION_TTL_SECONDS,
            metadata={"authentication": "dpapi-root-workstation-binding", "owner_id": binding.owner_id},
        )
        status = DeveloperAccessStatus(
            True, kind="root_workstation", developer_id=binding.owner_id,
            certificate_sha256=binding.certificate_sha256, fingerprint=binding.fingerprint,
            capabilities=tuple(session.granted_capabilities), session_expires_at=session.expires_at,
            root_key_id=binding.root_key_id,
        )
        try:
            self.security.audit.emit(
                actor=session.principal_id, action="developer.root_workstation.activate",
                resource="developer:internal/session", decision="allow",
                trust_domain=TrustDomain.DEVELOPER, reason="DPAPI_BOUND_OWNER_AUTHORITY_VERIFIED",
            )
        except Exception:
            self._retire_session(session)
            raise
    ...
    def ensure_root_workstation_session(self) -> Session | None:
        ...
        status = self.status()
        if status.authenticated and status.kind in {"root_owner", "root_workstation"}:
            with self._lock:
