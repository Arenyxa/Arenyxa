...
def run(manifest_path_value: str | os.PathLike[str], grant_payload: str) -> int:
    ...
    if len(raw_request) > 16 * 1024 * 1024:
        _worker_write("plugin request exceeds 16 MiB limit", file=sys.stderr)
        return 5
    try:
        request = json.loads(raw_request)
    except json.JSONDecodeError as exc:
        _worker_write(f"invalid plugin request: {exc}", file=sys.stderr)
        return 5
    namespace = runpy.run_path(str(entry), run_name="arenyxa_plugin")
    handler = namespace.get("handle")
    if not callable(handler):
        _worker_write("plugin must define handle(request)", file=sys.stderr)
        return 4
...
def main() -> int:
    if len(sys.argv) != 3:
        _worker_write("invalid worker arguments", file=sys.stderr)
        return 2
