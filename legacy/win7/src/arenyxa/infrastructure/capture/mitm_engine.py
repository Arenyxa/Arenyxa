...
class MitmEngine:
    ...
    def probe_version(executable: str) -> str:
        if not executable:
            return ""
        try:
            completed = subprocess.run(validated_argv([executable, "--version"]), capture_output=True, text=True, timeout=5, check=False)
    ...
    def start(self) -> MitmStatus:
        with self._lock:
            ...
            self._version = self.probe_version(executable)
            self._last_error = ""
