...
def _base(flow, event, protocol, phase):
...
def _apply_http(flow, phase, edited):
...
def _should_intercept(flow):
    return _INTERCEPT is not None and flowfilter.match(_INTERCEPT, flow)


async def _wait_for_control(flow, phase, payload, apply_callback):
...
class ArenyxaMitmBridge:
    ...
    def udp_error(self, flow):
        payload = _base(flow, "udp.error", "udp", "error")
        payload["payload"] = {"error": str(flow.error or "")}
        payload["_flow"] = flow
        _emit(payload)

    def dns_request(self, flow):
