...
class OfficeCoordinatorService:
    ...
    def identity_artifact(self) -> dict[str, Any]:
        with self._lock:
            if self._identity_artifact is None:
                raise _fail("COORDINATOR_NOT_STARTED", "Coordinator TLS identity has not been prepared")
            return json.loads(json.dumps(self._identity_artifact))
    ...
    def _build_tls_context(self) -> tuple[ssl.SSLContext, bytes]:
    ...
    def start_tls(self, host: str = "127.0.0.1", port: int = 0) -> tuple[str, int]:
        ...
        context, _cert_der = self._build_tls_context()
            ...
            def do_GET(self) -> None:
                ...
                try:
                    if self.path == "/v1/identity":
                        self._send(200, service.identity_artifact())
                    elif self.path == "/v1/health":
                        self._send(200, service.health())
                    else:
                        self._send(404, {"error": "not_found"})
                except Exception as exc:
                    self._send(400, {"error": getattr(exc, "code", type(exc).__name__), "message": str(exc)[:512]})
