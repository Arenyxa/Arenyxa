...
class WaterfallWidget(QWidget):
    def __init__(self, model: NetworkEventModel, theme, parent=None) -> None:
...
class NetworkPage(WorkspacePage):
    ...
    def __init__(self, context, theme, motion, parent=None) -> None:
        ...
        header = QHBoxLayout()
        header.addWidget(
            PageHeader("网络分析", "Browser Capture / Packet Capture / HAR / Replay / TLS / DNS"), 1
        )
        self.source = QComboBox()
        self.source.addItem("Browser Capture", CaptureSource.BROWSER)
        self.source.addItem("System Packet (tshark)", CaptureSource.SYSTEM)
        self.source.addItem("Import HAR", CaptureSource.HAR_IMPORT)
        self.source.addItem("Import PCAP / PCAPNG", CaptureSource.PCAP_IMPORT)
        ...
        self.packet_analysis = QPushButton("Packet Intelligence Workbench")
    ...
    def start_capture(self) -> None:
        ...
        try:
            ...
            if source is CaptureSource.HAR_IMPORT:
                ...
                self.model.replace([NetworkEventModel._as_row(event) for event in events])
    ...
    def update_status(self) -> None:
        ...
        self.capture_status.setText(
            ...
            f"Dropped {session.dropped_events:,} · {session.permission_state}"
        )
    ...
    def load_session(self, row: int) -> None:
        ...
        def completed(value: object) -> None:
            if token != self._session_load_token:
                return
            events = list(value) if isinstance(value, list) else []
            self.model.replace(events)
            self.inspectorChanged.emit("Capture Session", {"id": session_id, "visible_events": len(events)})
    ...
    def inspect_event(self, current, previous) -> None:
        del previous
    ...
    def _import_pcap_session(self, session: CaptureSession, source_path: Path) -> None:
        ...
        def run_import():
            ...
            events = engine.to_network_events(target, session, display_filter=display_filter, limit=limit)
            session.state = CaptureState.COMPLETED
            session.finished_at = utc_now()
            session.event_count = len(events)
        ...
        def completed(value: object) -> None:
            ...
            self.overview.setPlainText(json.dumps({
                ...
                "decoded_packets": len(events),
                "raw_bytes": session.bytes_captured,
                "display_filter": display_filter,
                "backend": "packet-analysis runtime",
            }, ensure_ascii=False, indent=2))
            self.statusMessage.emit(f"PCAP 已导入：{len(events):,} packets")
            self.context.nextgen.activity.publish(
                "capture-import",
                "PCAP imported",
                details={"session_id": session.id, "packets": len(events), "path": str(target_path)},
            )
            self.refresh_sessions()
