...
class MitmInterceptionPage(WorkspacePage):
    def __init__(self, context, theme, motion, parent=None) -> None:
        ...
        self.engine = MitmEngine(Path(context.paths.captures) / "mitm")
        self._pending_tokens: list[str] = []
        self._last_event_count = -1
        root = page_layout(self)
        header = QHBoxLayout()
        header.addWidget(PageHeader("MITM Workbench", "interception, replay, rules and advanced capture modes"), 1)
    ...
    def start_runtime(self) -> None:
        try:
            ...
            self.timer.start()
            self.statusMessage.emit("MITM Workbench started")
        except Exception as exc:
            QMessageBox.critical(self, "Unable to start MITM Workbench", str(exc))
        self.refresh_runtime()
    ...
    def stop_runtime(self) -> None:
        ...
        self.stop_button.setEnabled(False)
        self.refresh_runtime()
        self.statusMessage.emit("MITM Workbench stopped")
