...
class IntruderPayload:
    """Represent a fuzzing payload with its target position and replacement value."""
    position: int
    original: str
    replacement: str
...
class IntruderResult:
    """Represent the result of a single fuzzing attempt."""
    payload: str
    status: int | None
    response_bytes: int
    duration_ms: float
...
class Intruder:
    """
    Provide bounded Intruder-style automated fuzzing and parameter testing.
    Support attack strategies such as Sniper and Battering Ram.
    """
    MAX_PAYLOADS = 10000

    def generate_variants(self, template: str, positions: list[tuple[int, int]], payloads: list[str]) -> Iterable[str]:
        """
        Generate request variants from a template and marked positions.
        :param template: Original request string.
        :param positions: Marked offset ranges as ``(start, end)`` pairs.
        :param payloads: Replacement payload values.
        """
...
class MessageCodec:
    ...
    def transform(self, value: str, mode: str, operation: str) -> CodecResult:
        text = str(value)
        raw = text.encode("utf-8")
        if len(raw) > self.MAX_INPUT_BYTES:
            raise ValueError("Codec input exceeds the safety limit")
        ...
        if normalized_mode not in allowed_operations:
            raise ValueError("Unsupported codec mode")
        if normalized_operation not in allowed_operations[normalized_mode]:
            raise ValueError(f"Unsupported {normalized_mode} codec operation: {normalized_operation}")
        if normalized_mode == "url":
            output = quote(text, safe="") if normalized_operation == "encode" else unquote(text)
        elif normalized_mode == "base64":
            if normalized_operation == "encode":
                output = base64.b64encode(raw).decode("ascii")
            else:
                try:
                    output = base64.b64decode(text.encode("ascii"), validate=True).decode("utf-8", "replace")
            ...
            if normalized_operation == "encode":
                ...
                try:
                    output = bytes.fromhex(text).decode("utf-8", "replace")
            ...
            output = json.dumps(parsed, ensure_ascii=False, indent=2, sort_keys=True)
        output_bytes = len(output.encode("utf-8"))
        if output_bytes > self.MAX_OUTPUT_BYTES:
            raise ValueError("Codec output exceeds the safety limit")
        return CodecResult(normalized_mode, normalized_operation, output, len(raw), output_bytes)
...
class MessageComparer:
    ...
    def compare(self, left: str, right: str, *, context: int = 3) -> ComparisonResult:
        left_text = str(left)
        right_text = str(right)
        if len(left_text.encode("utf-8")) > self.MAX_INPUT_BYTES or len(right_text.encode("utf-8")) > self.MAX_INPUT_BYTES:
            raise ValueError("Comparer input exceeds the safety limit")
        left_lines = left_text.splitlines()
        right_lines = right_text.splitlines()
        diff_lines = list(difflib.unified_diff(left_lines, right_lines, fromfile="left", tofile="right", n=max(0, min(20, int(context))), lineterm=""))
        ...
        if len(diff.encode("utf-8")) > self.MAX_DIFF_BYTES:
            raise ValueError("Comparer output exceeds the safety limit")
        return ComparisonResult(left_text == right_text, len(left_lines), len(right_lines), changed, diff)
...
class PassiveSecurityAuditor:
    ...
    def audit_event(self, event: NetworkEvent) -> list[PassiveFinding]:
        ...
        if "text/html" in content_type:
            ...
            if "x-frame-options" not in response and "frame-ancestors" not in response.get("content-security-policy", ""):
                findings.append(PassiveFinding(
                    "CLICKJACKING_RISK", "medium", "X-Frame-Options is absent",
                    host, "Set X-Frame-Options: DENY or SAMEORIGIN to prevent clickjacking.",
                ))
            if "referrer-policy" not in response:
                findings.append(PassiveFinding(
                    "REFERRER_POLICY_MISSING", "info", "Referrer-Policy is absent",
                    host, "Set a Referrer-Policy to control how much referrer information is shared.",
                ))
            if "x-xss-protection" not in response:
                 findings.append(PassiveFinding(
                    "XSS_PROTECTION_MISSING", "info", "X-XSS-Protection is absent",
                    host, "Although deprecated, setting X-XSS-Protection: 0 or 1; mode=block provides defense-in-depth.",
                ))
        ...
        if "x-powered-by" in response:
            findings.append(PassiveFinding(
                "FINGERPRINTING_HEADER", "info", "X-Powered-By header is exposed",
                f"{host}: {response['x-powered-by'][:160]}", "Remove X-Powered-By to reduce information disclosure.",
            ))
        if "cache-control" not in response and ("authorization" in request or "cookie" in request):
            findings.append(PassiveFinding(
                "CACHE_CONTROL_MISSING", "low", "Cache-Control is absent for sensitive request",
                host, "Set Cache-Control: no-store or private for authenticated responses.",
            ))
        return findings
