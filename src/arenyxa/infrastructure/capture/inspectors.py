...
class TlsInspector:
    ...
    def inspect(host: str, port: int = 443, timeout: float = 10.0) -> TlsReport:
        ...
        # Make the certificate-validation contract explicit. create_default_context()
        # already provides these defaults, but pinning the invariant here prevents a
        # future MITM/diagnostic change from silently weakening TlsInspector.
...
class DnsAnalyzer:
    MAX_ADDRESSES = 64
    MAX_RECORDS_PER_TYPE = 64
    MAX_RECORD_TEXT_CHARS = 2048
    MAX_TOTAL_RECORD_TEXT_CHARS = 64 * 1024
    ...
    def resolve(host: str, port: int = 443) -> DnsReport:
        ...
        for family, socktype, proto, canonname, sockaddr in records:
            ...
            if len(addresses) >= DnsAnalyzer.MAX_ADDRESSES:
                break
        ...
        try:
            ...
            # 1232 bytes is the modern conservative EDNS UDP size and avoids
            # advertising oversized datagrams that are prone to fragmentation.
            if hasattr(resolver, "use_edns"):
                resolver.use_edns(edns=0, payload=DnsAnalyzer.EDNS_UDP_PAYLOAD)
            total_chars = 0
            bounded = False
            for record_type in ("A", "AAAA", "CNAME", "MX", "TXT", "NS"):
                try:
                    ...
                    values: list[str] = []
                    for item in answer:
                        if len(values) >= DnsAnalyzer.MAX_RECORDS_PER_TYPE:
                            bounded = True
                            break
                        text = str(item.to_text())
                        if len(text) > DnsAnalyzer.MAX_RECORD_TEXT_CHARS:
                            text = text[: DnsAnalyzer.MAX_RECORD_TEXT_CHARS]
                            bounded = True
                        remaining = DnsAnalyzer.MAX_TOTAL_RECORD_TEXT_CHARS - total_chars
                        if remaining <= 0:
                            bounded = True
                            break
                    ...
                    report.records[record_type] = values
                except Exception as exc:                                                                 
                    report.errors[record_type] = type(exc).__name__
            if bounded:
                report.errors["bounds"] = "response_truncated"
        except ImportError:
