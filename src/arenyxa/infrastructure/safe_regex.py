...
def _worker(connection: Any) -> None:
    """Serve bounded regex requests in an isolated process.

    The worker is intentionally persistent.  Starting a fresh Python interpreter on
    Windows can take longer than the user-requested regex execution deadline under
    CPU or antivirus pressure.  A startup handshake therefore separates process
    bootstrap latency from the regex execution deadline while keeping the actual
    regex operation hard-killable.
    """
    try:
        ...
        while True:
            ...
            if not isinstance(request, tuple) or len(request) != 4:
                connection.send(("error", "REGEX_PROTOCOL_INVALID", "invalid regex worker request"))
                continue
            operation, pattern, text, replacement = request
            try:
                if operation == "search":
            ...
            except (RuntimeError, ValueError, TypeError, OverflowError, RecursionError) as exc:
...
_SANDBOX_PROCESS: Any | None = None
_SANDBOX_CONNECTION: Any | None = None


def _stop_sandbox(*, graceful: bool = False) -> None:
    global _SANDBOX_PROCESS, _SANDBOX_CONNECTION
    process = _SANDBOX_PROCESS
    connection = _SANDBOX_CONNECTION
...
def _ensure_sandbox() -> tuple[Any, Any]:
    global _SANDBOX_PROCESS, _SANDBOX_CONNECTION
    process = _SANDBOX_PROCESS
    connection = _SANDBOX_CONNECTION
    ...
    if not parent.poll(STARTUP_TIMEOUT_SECONDS):
        ...
        if process.is_alive():
            ...
            process.join(timeout=0.5)
        raise _regex_error(
            "REGEX_SANDBOX_STARTUP_TIMEOUT",
            "Regex sandbox failed to become ready within the startup limit",
    ...
    try:
        ...
        parent.close()
        process.join(timeout=0.5)
        raise _regex_error("REGEX_SANDBOX_STARTUP_FAILED", "Regex sandbox exited during startup") from exc
    if ready != ("ready",):
        parent.close()
        if process.is_alive():
            process.terminate()
        process.join(timeout=0.5)
        raise _regex_error("REGEX_SANDBOX_STARTUP_FAILED", "Regex sandbox returned an invalid startup handshake")
    _SANDBOX_PROCESS = process
    _SANDBOX_CONNECTION = parent
...
def _run(operation: str, pattern: str, text: str, replacement: str, timeout_seconds: float) -> Any:
    ...
    with _SANDBOX_LOCK:
        process, connection = _ensure_sandbox()
        try:
            connection.send((operation, pattern, text, replacement))
        except (BrokenPipeError, EOFError, OSError) as exc:
            _stop_sandbox()
            raise _regex_error("REGEX_EXECUTION_FAILED", "Regex worker became unavailable before execution") from exc
