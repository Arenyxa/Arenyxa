...
def default_roles() -> dict[str, dict[str, Any]]:
    return {
        ...
        "administrator": {
            ...
            "permissions": ["enterprise.account.manage", "enterprise.audit.read", "enterprise.enrollment.manage", "enterprise.device.manage", "enterprise.workspace.manage", "enterprise.approval.manage", "enterprise.quota.manage", "dataset.read", "dataset.write", "dataset.export", "workflow.execute", "workflow.publish", "enterprise.capture.run", "schedule.manage", "worker.use", "enterprise.worker.manage", "enterprise.server.manage", "enterprise.remote_ops", "enterprise.proxy.manage", "enterprise.mitm.manage", "enterprise.packet.analyze", "enterprise.network.observe"],
        },
        "network_security_admin": {
            "id": "network_security_admin", "title": "Network Security Administrator", "builtin": True,
            "permissions": ["enterprise.audit.read", "dataset.read", "dataset.write", "dataset.export",
                            "enterprise.capture.run", "enterprise.proxy.manage", "enterprise.mitm.manage",
                            "enterprise.packet.analyze", "enterprise.network.observe"],
        },
        "network_analyst": {
            "id": "network_analyst", "title": "Network Analyst", "builtin": True,
            "permissions": ["enterprise.audit.read", "dataset.read", "dataset.export",
                            "enterprise.packet.analyze", "enterprise.network.observe"],
        },
        "proxy_operator": {
            "id": "proxy_operator", "title": "Proxy Operator", "builtin": True,
            "permissions": ["dataset.read", "enterprise.capture.run", "enterprise.proxy.manage",
                            "enterprise.mitm.manage", "enterprise.network.observe"],
        },
        "security_auditor": {
            "id": "security_auditor", "title": "Security Auditor", "builtin": True,
            "permissions": ["enterprise.audit.read", "dataset.read", "dataset.export",
                            "enterprise.packet.analyze", "enterprise.network.observe"],
        },
        "operator": {
...
def ensure_builtin_role_catalog(payload: dict[str, Any]) -> bool:
    """Upgrade immutable built-in role presets in memory without weakening custom RBAC.

    The encrypted payload schema remains v1; this is a role-catalog migration.  The next
    authenticated Vault save persists the catalog alongside the normal mutation.
    """
    ...
    for role_id, expected in defaults.items():
        ...
        if current != expected:
            roles[role_id] = {
                ...
                "builtin": True,
                "permissions": list(expected["permissions"]),
            }
            changed = True
    return changed
...
class EnterpriseVault:
    ...
    def open(self, vault_passphrase: str) -> EnterpriseVaultHandle:
        ...
        try:
            ...
            ensure_builtin_role_catalog(payload)
    ...
    def envelope_health(self, handle: EnterpriseVaultHandle | None = None) -> dict[str, Any]:
        """Return non-secret structural/integrity metadata for the encrypted Vault envelope.

        This intentionally does not expose salts, nonces, ciphertext, wrapped keys, or key
        material.  When an unlocked handle is supplied, the report also verifies that the
        on-disk passphrase envelope is the same envelope authenticated by that handle.
        """
        if self.path.is_symlink():
            raise _fail("ENTERPRISE_VAULT_UNSAFE_PATH", "Enterprise Vault cannot be a symbolic link")
        raw = read_bytes_limited(self.path, MAX_VAULT_BYTES)
        ...
        encrypted_payload = outer.get("payload")
        if not enterprise_id or not isinstance(kdf, dict) or not isinstance(wrapped, dict) or not isinstance(encrypted_payload, dict):
            raise _fail("ENTERPRISE_VAULT_INVALID", "Enterprise Vault envelope fields are invalid")
        if set(kdf) != {"algorithm", "salt", "n", "r", "p"} or kdf.get("algorithm") != "scrypt":
            raise _fail("ENTERPRISE_KDF_INVALID", "Enterprise Vault KDF is invalid")
        n = int(kdf.get("n", 0))
        if n < MIN_SCRYPT_N or n > MAX_SCRYPT_N or n & (n - 1) or int(kdf.get("r", 0)) != SCRYPT_R or int(kdf.get("p", 0)) != SCRYPT_P:
            raise _fail("ENTERPRISE_KDF_INVALID", "Enterprise Vault KDF work factor is invalid")
        _b64u_decode(str(kdf.get("salt", "")), max_bytes=64)
        if set(wrapped) != {"algorithm", "nonce", "ciphertext"} or wrapped.get("algorithm") != "AES-256-GCM":
            raise _fail("ENTERPRISE_VAULT_INVALID", "Enterprise Vault wrapped-key cipher is invalid")
        _b64u_decode(str(wrapped.get("nonce", "")), max_bytes=32)
        _b64u_decode(str(wrapped.get("ciphertext", "")), max_bytes=128)
        if set(encrypted_payload) != {"algorithm", "nonce", "ciphertext", "sha256"} or encrypted_payload.get("algorithm") != "AES-256-GCM":
            raise _fail("ENTERPRISE_VAULT_INVALID", "Enterprise Vault payload cipher is invalid")
        _b64u_decode(str(encrypted_payload.get("nonce", "")), max_bytes=32)
        payload_ciphertext = _b64u_decode(str(encrypted_payload.get("ciphertext", "")), max_bytes=MAX_VAULT_BYTES)
        digest_valid = hmac.compare_digest(
            hashlib.sha256(payload_ciphertext).hexdigest(), str(encrypted_payload.get("sha256", ""))
        )
        if not digest_valid:
            raise _fail("ENTERPRISE_VAULT_INTEGRITY", "Enterprise Vault ciphertext digest is invalid")
        binding = self._envelope_binding(enterprise_id, kdf, wrapped)
        handle_binding_valid: bool | None = None
        if handle is not None:
            if str(handle.payload.get("enterprise_id", "")) != enterprise_id:
                raise _fail("ENTERPRISE_VAULT_INTEGRITY", "Unlocked Vault handle belongs to another Enterprise")
            handle_binding_valid = bool(handle._envelope_binding) and hmac.compare_digest(binding, handle._envelope_binding)
            if not handle_binding_valid:
                raise _fail("ENTERPRISE_VAULT_INTEGRITY", "Vault passphrase envelope changed while this session was unlocked")
        return {
            "schema": str(outer["schema"]),
            "version": int(outer["version"]),
            "enterprise_id": enterprise_id,
            "file_bytes": len(raw),
            "kdf": {"algorithm": "scrypt", "n": n, "r": SCRYPT_R, "p": SCRYPT_P},
            "wrapped_key_algorithm": "AES-256-GCM",
            "payload_algorithm": "AES-256-GCM",
            "ciphertext_sha256_valid": True,
            "unlocked_binding_valid": handle_binding_valid,
        }

    def rewrap_passphrase(
        self, handle: EnterpriseVaultHandle, *, current_passphrase: str, new_passphrase: str,
        scrypt_n: int = DEFAULT_SCRYPT_N,
    ) -> None:
        """Rotate only the passphrase envelope while preserving the random Vault data key.

        The current passphrase is cryptographically verified, the active handle must still
        match the on-disk envelope, and the replacement is written atomically.
        """
        _validate_passphrase(new_passphrase)
        with handle._lock:
            if self.path.is_symlink():
                raise _fail("ENTERPRISE_VAULT_UNSAFE_PATH", "Enterprise Vault cannot be a symbolic link")
            outer = _load_object_bytes(read_bytes_limited(self.path, MAX_VAULT_BYTES))
            expected = {"schema", "version", "enterprise_id", "kdf", "wrapped_key", "payload"}
            if set(outer) != expected or outer.get("schema") != VAULT_SCHEMA or outer.get("version") != 1:
                raise _fail("ENTERPRISE_VAULT_INVALID", "Enterprise Vault envelope schema is invalid")
            enterprise_id = str(outer.get("enterprise_id", ""))
            if enterprise_id != str(handle.payload.get("enterprise_id", "")):
                raise _fail("ENTERPRISE_VAULT_INTEGRITY", "Enterprise ID does not match unlocked Vault handle")
            current_kdf = outer.get("kdf")
            current_wrapped = outer.get("wrapped_key")
            if not isinstance(current_kdf, dict) or not isinstance(current_wrapped, dict):
                raise _fail("ENTERPRISE_VAULT_INVALID", "Enterprise Vault key envelope is invalid")
            current_binding = self._envelope_binding(enterprise_id, current_kdf, current_wrapped)
            if not handle._envelope_binding or not hmac.compare_digest(current_binding, handle._envelope_binding):
                raise _fail("ENTERPRISE_VAULT_INTEGRITY", "Vault passphrase envelope changed while this session was unlocked")

            # Verify the old passphrase against the authenticated wrapped data key.
            try:
                salt = _b64u_decode(str(current_kdf.get("salt", "")), max_bytes=64)
                old_kek = _derive_key(current_passphrase, salt, int(current_kdf.get("n", 0)))
                try:
                    old_data_key = AESGCM(old_kek).decrypt(
                        _b64u_decode(str(current_wrapped.get("nonce", "")), max_bytes=32),
                        _b64u_decode(str(current_wrapped.get("ciphertext", "")), max_bytes=128),
                        self._outer_aad(enterprise_id, current_kdf),
                    )
                finally:
                    mutable = bytearray(old_kek)
                    for index in range(len(mutable)):
                        mutable[index] = 0
            except InvalidTag as exc:
                raise _fail("ENTERPRISE_VAULT_UNLOCK_FAILED", "Current Vault passphrase is invalid") from exc
            live_key = handle.data_key()
            try:
                if len(old_data_key) != 32 or not hmac.compare_digest(old_data_key, live_key):
                    raise _fail("ENTERPRISE_VAULT_INTEGRITY", "Wrapped Vault data key does not match unlocked session")
            finally:
                mutable_old = bytearray(old_data_key)
                for index in range(len(mutable_old)):
                    mutable_old[index] = 0

            new_salt = secrets.token_bytes(16)
            new_kdf = {"algorithm": "scrypt", "salt": _b64u_encode(new_salt), "n": int(scrypt_n), "r": SCRYPT_R, "p": SCRYPT_P}
            new_kek = _derive_key(new_passphrase, new_salt, int(scrypt_n))
            new_nonce = secrets.token_bytes(12)
            try:
                new_ciphertext = AESGCM(new_kek).encrypt(new_nonce, live_key, self._outer_aad(enterprise_id, new_kdf))
            finally:
                mutable = bytearray(new_kek)
                for index in range(len(mutable)):
                    mutable[index] = 0
                mutable_live = bytearray(live_key)
                for index in range(len(mutable_live)):
                    mutable_live[index] = 0
            new_wrapped = {
                "algorithm": "AES-256-GCM",
                "nonce": _b64u_encode(new_nonce),
                "ciphertext": _b64u_encode(new_ciphertext),
            }
            replacement = dict(outer)
            replacement["kdf"] = new_kdf
            replacement["wrapped_key"] = new_wrapped
            atomic_write_json(self.path, replacement, ensure_ascii=False, indent=2, mode=0o600)
            handle._envelope_binding = self._envelope_binding(enterprise_id, new_kdf, new_wrapped)
