...
class AuthThrottleIntegrityError(RuntimeError):
    """Marker type for AuthThrottleIntegrityError."""
...
class BoundedWindowRateLimiter:
    def __init__(self, max_buckets: int) -> None:
        ...
        self._overflow_window: tuple[float, int] = (time.monotonic(), 0)
        self._overflow_rejections = 0

    @staticmethod
    def _advance_window(started: float, count: int, now: float, window: float) -> tuple[float, int]:
        if now - float(started) >= window:
            return now, 0
        return float(started), int(count)

    def _prune_expired_locked(self, now: float, window: float) -> None:
        expired = [key for key, (started, _count) in self._buckets.items() if now - float(started) >= window]
        for key in expired:
    ...
    def allow(self, key: str, *, limit: int, window_seconds: float = 60.0) -> bool:
        ...
        with self._lock:
            existing = self._buckets.get(normalized)
            if existing is not None:
                started, count = self._advance_window(existing[0], existing[1], now, window)
                count += 1
                self._buckets[normalized] = (started, count)
                self._buckets.move_to_end(normalized)
                return count <= ceiling

            self._prune_expired_locked(now, window)
            if len(self._buckets) < self.max_buckets:
                self._buckets[normalized] = (now, 1)
                return True

            started, count = self._advance_window(self._overflow_window[0], self._overflow_window[1], now, window)
            count += 1
            self._overflow_window = (started, count)
            overflow_ceiling = max(ceiling, min(ceiling * 4, ceiling + self.max_buckets))
            allowed = count <= overflow_ceiling
            if not allowed:
    ...
    def snapshot(self) -> dict[str, int]:
        with self._lock:
            return {
                ...
                "overflow_window_count": int(self._overflow_window[1]),
                "overflow_rejections": int(self._overflow_rejections),
            }
