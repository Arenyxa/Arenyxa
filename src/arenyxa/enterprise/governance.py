...
class EnterpriseGovernanceService:
        ...
        *,
        ...
        def mutate(state: dict[str, Any], vault: dict[str, Any]) -> None:
            ...
            state["resources"][resource_id] = {
                ...
                "quota": quota_map, "grants": {}, "zero_trust_policy": ZeroTrustPolicy().as_dict(),
                "created_at": utc_now(), "updated_at": utc_now(),
            }
    ...
    def set_zero_trust_policy(self, resource_id: str, policy: Mapping[str, Any] | ZeroTrustPolicy) -> dict[str, Any]:
        resource_key = str(resource_id).strip()
        if not resource_key or len(resource_key) > 256:
            raise _fail("GOVERNANCE_ID_INVALID", "resource identifier is invalid")
        ...
        def mutate(state: dict[str, Any], _vault: dict[str, Any]) -> None:
            state = _state(state)
            resource = state["resources"].get(resource_key)
            if not isinstance(resource, dict):
                raise _fail("GOVERNANCE_RESOURCE_MISSING", "Enterprise resource does not exist")
            resource["zero_trust_policy"] = normalized.as_dict()
            resource["updated_at"] = utc_now()
            result.update(resource["zero_trust_policy"])

        self.identity.mutate_extension(
            "governance", "enterprise.policy.modify", resource_key,
            "enterprise.zero_trust.policy.update", mutate, step_up=True,
        )
        return dict(result)
    ...
    def authorize_operation(
        ...
        zero_trust_policy = ZeroTrustPolicy.from_mapping(resource.get("zero_trust_policy"))
        zero_trust = ZeroTrustEvaluator.evaluate(zero_trust_policy, access_context)
        if not zero_trust.allowed:
            raise _fail(
                zero_trust.code,
                "Enterprise operation was denied by the dynamic Zero Trust access policy",
                permission=permission, resource_id=resource_id, reasons=list(zero_trust.reasons),
                risk_score=zero_trust.risk_score,
            )
        status = self.identity.status()
