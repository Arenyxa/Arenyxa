...
class AppPaths:
    ...
    def log_file(self) -> Path:
        """Canonical destination for all new Arenyxa log writes."""

        return self.logs / LOG_FILENAME

    @property
    def readable_log_file(self) -> Path:
        """Readable log path with a legacy-name fallback for migrated installations."""
...
class AppSettings:
    ...
    def load(cls, path: Path) -> AppSettings:
        ...
        for name in (
            ...
            "inspector_collapsed", "diagnostics_include_paths", "developer_mode", "developer_direct_shell_enabled",
        ...
        if values.get("experience_profile") not in {"", "personal", "power", "professional", "developer", "enterprise", "root_developer"}:
            values["experience_profile"] = defaults.experience_profile
        if values.get("personal_scenario") not in {
            "", "website_analysis", "api_debugging", "network_diagnostics",
            "data_collection", "security_learning",
        }:
            values["personal_scenario"] = defaults.personal_scenario
        ...
        if not values.get("developer_mode", False):
            ...
            values["developer_direct_shell_enabled"] = False
