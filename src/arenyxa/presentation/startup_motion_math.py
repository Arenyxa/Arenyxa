...
def startup_progress_duration_ms(start_percent: float, target_percent: float) -> int:
    """Return a compact, distance-aware duration for real bootstrap milestones.

    The current runtime reports many more authoritative stages than the older sparse 10/30/50/70/85
    sequence.  Shorter transitions preserve a continuous loading feel without turning
    cosmetic progress into measurable startup latency.
    """
    start = max(0.0, min(100.0, float(start_percent)))
    target = max(start, min(100.0, float(target_percent)))
    distance = target - start
    if distance <= 0.0:
        return 0
    return int(round(max(70.0, min(150.0, 52.0 + distance * 3.8))))
