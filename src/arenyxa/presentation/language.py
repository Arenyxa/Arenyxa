...
from arenyxa.presentation.language_enterprise import ENTERPRISE_PHRASES
...
EN = {
    ...
    "nav.automation": "Automation Center", "nav.recovery": "Recovery & Health", "nav.advanced": "Advanced Platform", "nav.studio": "Web Intelligence", "nav.visualization": "Visualization Studio",
...
ZH_CN = {
    **EN,
    "nav.task_center": "任务中心", "nav.dashboard": "仪表盘", "nav.tasks": "抓取任务", "nav.capture": "捕获", "nav.search": "搜索中心", "nav.data": "数据管理",
    "nav.network": "网络分析", "nav.proxy": "Proxy Suite", "nav.api_security": "API Security Lab", "nav.forensics": "Forensics Lab", "nav.traffic_ai": "AI Traffic Intelligence", "nav.server_ops": "Fleet Control", "nav.mitm_proxy": "MITM Proxy", "nav.professional": "专业工具中心", "nav.extraction": "Extraction Lab", "nav.crawler": "Crawler Lab", "nav.workflow": "Flow Designer", "nav.automation": "Automation Engine", "nav.recovery": "运行恢复", "nav.advanced": "高级平台", "nav.studio": "Web Intelligence",
...
PROFESSIONAL_NAV_NAMES = {
    "nav.network": "Network Analysis",
    "nav.proxy": "Proxy Suite",
    "nav.api_security": "API Security Lab",
    "nav.forensics": "Forensics Lab",
    "nav.traffic_ai": "AI Traffic Intelligence",
    "nav.mitm_proxy": "MITM Proxy",
    "nav.extraction": "Extraction Lab",
    "nav.crawler": "Crawler Lab",
    "nav.studio": "Web Intelligence",
    "nav.workflow": "Flow Designer",
    "nav.automation": "Automation Engine",
    "nav.server_ops": "Fleet Control",
    "nav.protocol": "Protocol Intelligence",
    "nav.security_center": "Security Center",
    "nav.server": "Server",
    "nav.workers": "Workers",
    "nav.platform_jobs": "Jobs",
    "nav.storage": "Storage & Database",
    "nav.audit": "Audit",
    "nav.diagnostics": "Diagnostics",
    "nav.performance": "Performance",
    "nav.developer_center": "Developer",
}
EN.update(PROFESSIONAL_NAV_NAMES)
ZH_CN.update(PROFESSIONAL_NAV_NAMES)
for _locale in NATIVE_OVERRIDES:
    NATIVE_OVERRIDES[_locale].update(PROFESSIONAL_NAV_NAMES)
...
def _load_locale_catalogs() -> None:
    ...
    for path in locale_root.glob("*.json"):
        ...
        if not isinstance(payload, dict):
            continue
        table = TRANSLATIONS.setdefault(path.stem, {**EN})
        table.update({str(key): str(value) for key, value in payload.items() if isinstance(value, str)})
...
PHRASES.update({
    ...
    "Direct Shell（允许 PowerShell / CMD / Persistent Shell；仅 Developer Mode）": "Direct Shell (allow PowerShell / CMD / Persistent Shell; Developer Mode only)",
    "完整 Shell 使用当前 Windows 用户权限执行；高风险命令仍会额外确认并写入终端审计。": "Full shell commands run with the current Windows user's permissions; high-risk commands still require extra confirmation and are recorded in the terminal audit.",
...
PHRASES.update({
    ...
    "例如：分析这个 PCAP、检查 DNS 隧道、看看电脑连接了哪些服务器、调试 API":
        "For example: analyze this PCAP, inspect DNS tunneling, see which servers this computer connects to, or debug an API",
})
...
PHRASES.update({
    ...
    "使用此模式": "Use this mode",
    "企业能力只在对应安全后端真实可用时开放，不使用占位权限或伪造 Enterprise Identity / Vault / RBAC。": "Enterprise capabilities are exposed only when their security backends are actually available; placeholder permissions and simulated Enterprise Identity / Vault / RBAC are not used.",
