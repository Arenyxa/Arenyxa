...
class PacketIntelligenceWorkbenchDialog(QDialog):
    def __init__(self, capture_path: Path, selected_event: dict[str, Any] | None = None, parent=None) -> None:
        ...
        self.intelligence_button = QPushButton("Intelligence Report")
        self.forensics_button = QPushButton("Forensic Correlation")
        self.evidence_fusion_button = QPushButton("Passive Evidence Fusion")
        self.intelligence_button.setProperty("primary", True)
        ...
        for button in (
            ...
            self.forensics_button,
            self.evidence_fusion_button,
            self.packet_button,
            self.expert_button,
            self.follow_button,
            self.export_button,
        ...
        self.custom_tap.setPlaceholderText("External dissector statistics tap, e.g. sip,stat or smb2,srt")
        ...
        self.intelligence_view = self._view()
        self.forensics_view = self._view()
        self.evidence_fusion_view = self._view()
        ...
        self.tabs.addTab(self.intelligence_view, "Intelligence Report")
        self.tabs.addTab(self.forensics_view, "Forensic Correlation")
        self.tabs.addTab(self.evidence_fusion_view, "Passive Evidence Fusion")
        ...
        self.intelligence_button.clicked.connect(self.load_intelligence_report)
        self.forensics_button.clicked.connect(self.load_forensic_correlation)
        self.evidence_fusion_button.clicked.connect(self.load_passive_evidence_fusion)
    ...
    def load_capabilities(self) -> None:
        self._background(
            lambda: {
                "capabilities": asdict(self.engine.capabilities()),
                "protocol_coverage": self.engine.protocol_coverage(),
            },
    ...
    def load_intelligence_report(self) -> None:
        expression = self.display_filter.text().strip()
        profile = self._profile()

        def report() -> dict[str, Any]:
            records = self.engine.packet_summaries(self.capture_path, expression, 100000, profile)
            return PacketIntelligenceReporter().analyze(records, limit=100000).snapshot()
    ...
    def load_forensic_correlation(self) -> None:
        expression = self.display_filter.text().strip()
        self._background(
            lambda: self.engine.forensic_summary(
                self.capture_path, display_filter=expression, limit=200000, profile=self._profile(),
            ),
            self.forensics_view,
            7,
            lambda value: json.dumps(value, ensure_ascii=False, indent=2, default=str),
    ...
    def load_passive_evidence_fusion(self) -> None:
        zeek_paths, _ = QFileDialog.getOpenFileNames(
            self,
            "Select Zeek JSON Logs (optional)",
            "",
            "JSON / JSONL (*.json *.jsonl *.log);;All Files (*)",
        )
        suricata_path, _ = QFileDialog.getOpenFileName(
            self,
            "Select Suricata EVE JSON (optional)",
            "",
            "EVE JSON / JSONL (*.json *.jsonl *.log);;All Files (*)",
        )
        if not zeek_paths and not suricata_path:
            QMessageBox.information(
                self,
                "Passive Evidence Fusion",
                "Select Zeek JSON logs, a Suricata EVE JSON file, or both. The current capture is always retained as Arenyxa packet evidence.",
            )
            return
        expression = self.display_filter.text().strip()
        self._background(
            lambda: self.engine.fuse_passive_evidence(
                self.capture_path,
                display_filter=expression,
                profile=self._profile(),
                zeek_json_paths=tuple(Path(path) for path in zeek_paths),
                suricata_eve_path=Path(suricata_path) if suricata_path else None,
            ),
            self.evidence_fusion_view,
            8,
            lambda value: json.dumps(value, ensure_ascii=False, indent=2, default=str),
        )
    ...
    def load_catalog(self) -> None:
        def catalog() -> dict[str, Any]:
            native = [{**row, "source": "native"} for row in self.engine.native_protocol_catalog()]
            external = self.engine.protocol_catalog() if self.engine.available else []
            fields = self.engine.field_catalog(limit=5000) if self.engine.available else []
            return {
                "coverage": self.engine.protocol_coverage(),
                "native_protocols": native,
                "external_protocols": external,
