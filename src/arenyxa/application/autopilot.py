...
class ExperienceStore:
    ...
    def _prune_table(connection: sqlite3.Connection, table: str, maximum: int) -> None:
        ...
        count_statement = "SELECT COUNT(*) FROM " + quoted
        count = int(connection.execute(count_statement).fetchone()[0])
        excess = count - maximum
        if excess <= 0:
            return
        delete_statement = (
            "DELETE FROM " + quoted + " WHERE id IN (SELECT id FROM " + quoted + " ORDER BY id ASC LIMIT ?)"
        )
        connection.execute(delete_statement, (excess,))
