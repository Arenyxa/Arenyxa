"""Bounded external-process and persistent shell session runtime for Developer Mode."""
...
class TerminalMode(StrEnum):
    """Supported bounded terminal execution modes."""
    ...
    PYTHON_SESSION = "python-session"


@dataclass(frozen=True, slots=True)
class TerminalLaunch:
    """Resolved immutable launch specification for an external terminal process."""
    mode: TerminalMode
    executable: str
    arguments: tuple[str, ...]
    cwd: Path
...
class TerminalResult:
    """Immutable completion status for a terminal process."""
    exit_code: int
...
class TerminalSession:
    """Application-managed terminal session with bounded output, timeout, and process-tree control."""
    ...
    def __init__(
        ...
        *,
        ...
        self._active_mode: TerminalMode | None = None
        self._active_persistent = False
        self._finished_event = threading.Event()
        self._finished_event.set()

    @property
    def cwd(self) -> Path:
        """Return the current sandboxed project working directory."""
        with self._lock:
            return self._cwd

    @property
    def timeout_seconds(self) -> float:
        """Return the active external-command timeout in seconds."""
        with self._lock:
            return self._timeout_seconds

    @property
    def is_running(self) -> bool:
        """Return whether an external terminal process is running."""
        with self._lock:
            return self._process is not None and self._process.poll() is None

    @property
    def active_mode(self) -> TerminalMode | None:
        """Return the active persistent terminal mode, if any."""
        with self._lock:
            return self._active_mode

    @property
    def active_persistent(self) -> bool:
        """Return whether a persistent shell session is active."""
        with self._lock:
            return bool(self._active_persistent and self._process is not None and self._process.poll() is None)

    def set_timeout(self, seconds: float) -> float:
        """Validate and update the external-command timeout."""
    ...
    def redact_command(command: str) -> str:
        """Redact sensitive command material before display or logging."""
        result = command
    ...
    def remember(self, command: str) -> None:
        """Append a command to bounded in-memory terminal history."""
    ...
    def set_cwd(self, raw_path: str | Path) -> Path:
        """Resolve and set a working directory confined to the project root."""
    ...
    def list_directory(self, raw_path: str | Path = ".", *, limit: int = 500) -> list[dict[str, object]]:
        """List a confined project directory using bounded metadata."""
        candidate = Path(raw_path).expanduser()
        with self._lock:
            base = self._cwd
        if not candidate.is_absolute():
    ...
    def set_environment(self, name: str, value: str) -> None:
        """Set one session-scoped environment variable after validation."""
    ...
    def unset_environment(self, name: str) -> bool:
        """Remove one session-scoped environment variable."""
        if not _ENV_NAME.fullmatch(name):
            raise ValueError("invalid environment variable name")
        with self._lock:
    ...
    def environment_items(self, contains: str = "", *, mask_secrets: bool = True) -> list[tuple[str, str]]:
        """Return redacted session environment entries matching a filter."""
    ...
    def readonly_sql(database: Path, query: str, *, limit: int = 500) -> dict[str, object]:
        """Execute a bounded read-only SQL query against the local store."""
    ...
    def which(self, executable: str) -> str | None:
        """Resolve an executable using the terminal session environment."""
    ...
    def build_launch(self, command: str, mode: TerminalMode | str) -> TerminalLaunch:
        """Build a validated process launch specification for a terminal mode."""
        ...
        if selected == TerminalMode.DIRECT:
            return self._build_direct_launch(command, selected, cwd, risk)
        if selected in {TerminalMode.PYTHON, TerminalMode.PYTHON_SESSION}:
            return self._build_python_launch(command, selected, cwd, risk)
        if selected in {TerminalMode.POWERSHELL, TerminalMode.POWERSHELL_SESSION}:
            return self._build_powershell_launch(command, selected, cwd, risk)
        return self._build_cmd_launch(command, selected, cwd, risk)

    def _build_direct_launch(
        self, command: str, mode: TerminalMode, cwd: Path, risk: str | None
    ) -> TerminalLaunch:
        ...
        return TerminalLaunch(
            mode=mode, executable=executable, arguments=tuple(arguments[1:]), cwd=cwd,
            display=command, encoding=encoding, risk_reason=risk,
        )

    @staticmethod
    def _build_python_launch(
        command: str, mode: TerminalMode, cwd: Path, risk: str | None
    ) -> TerminalLaunch:
        persistent = mode == TerminalMode.PYTHON_SESSION
        arguments = ("-u", "-i") if persistent else ("-u", "-c", command)
        return TerminalLaunch(
            mode=mode, executable=sys.executable, arguments=arguments, cwd=cwd,
            display=command, encoding="utf-8", risk_reason=risk, persistent=persistent,
        )

    def _build_powershell_launch(
        self, command: str, mode: TerminalMode, cwd: Path, risk: str | None
    ) -> TerminalLaunch:
        ...
        persistent = mode == TerminalMode.POWERSHELL_SESSION
        arguments = (
            ("-NoLogo", "-NoProfile", "-NoExit", "-Command", prefix)
            if persistent
            else ("-NoLogo", "-NoProfile", "-NonInteractive", "-Command", prefix + command)
        )
        return TerminalLaunch(
            mode=mode, executable=executable, arguments=arguments, cwd=cwd, display=command,
            encoding="utf-8", risk_reason=risk, persistent=persistent,
        )

    def _build_cmd_launch(
        self, command: str, mode: TerminalMode, cwd: Path, risk: str | None
    ) -> TerminalLaunch:
        if os.name != "nt":
        ...
        persistent = mode == TerminalMode.CMD_SESSION
        arguments = (
            ("/D", "/Q", "/K", "chcp 65001>nul")
            if persistent
            else ("/D", "/S", "/C", "chcp 65001>nul & " + command)
        )
        return TerminalLaunch(
            mode=mode, executable=executable, arguments=arguments, cwd=cwd, display=command,
            encoding="utf-8", risk_reason=risk, persistent=persistent,
    ...
    def detect_risk(command: str, mode: TerminalMode | str) -> str | None:
        """Classify shell input that requires explicit high-risk confirmation."""
        ...
        selected = TerminalMode(mode)
        if selected in {TerminalMode.POWERSHELL, TerminalMode.CMD, TerminalMode.POWERSHELL_SESSION, TerminalMode.CMD_SESSION} and any(
    ...
    def start(
        ...
        """Start a bounded external or persistent terminal process."""
        with self._lock:
            ...
            self._active_mode = launch.mode
            self._active_persistent = bool(launch.persistent)
        ...
        try:
            ...
            if stream is not None:
                try:
                    ...
                    record_current_exception(__name__, 'TerminalSession._read_process:623')
            input_stream = process.stdin
            if input_stream is not None and not input_stream.closed:
                try:
                    input_stream.close()
                except OSError:
                    record_current_exception(__name__, 'TerminalSession._read_process:629')
            try:
    ...
    def send_input(self, text: str, *, append_newline: bool = True) -> bool:
        """Send one line of input to the active process without shell expansion."""
    ...
    def close_input(self) -> bool:
        """Close standard input for the active external process."""
        with self._lock:
            process = self._process
            stream = None if process is None else process.stdin
        if process is None or process.poll() is not None or stream is None or stream.closed:
    ...
    def request_stop(self) -> bool:
        ...
        """Request cooperative termination of the active process."""
        with self._lock:
            process = self._process
    ...
    def stop(self) -> bool:
        ...
        """Terminate the active process within the configured shutdown bounds."""
        with self._lock:
            process = self._process
            if process is None or process.poll() is not None:
                return False
        ...
        try:
            if os.name == "nt":
                ...
                try:
                    ...
                    record_current_exception(__name__, 'TerminalSession._terminate_specific:763')
                try:
                    subprocess.run(
            ...
            try:
                ...
                record_current_exception(__name__, 'TerminalSession._terminate_specific:789')
    ...
    def wait(self, timeout: float | None = None) -> bool:
        """Wait for the active process and return its terminal result."""
        return self._finished_event.wait(timeout)

    def close(self) -> None:
        """Release terminal process resources and persistent session state."""
        self.stop()
