...
HARDWARE_ROOT_SCHEMA = "arenyxa.developer-root-trust/v2"
HARDWARE_ROOT_SCHEMA_V3 = "arenyxa.developer-root-trust/v3"
HARDWARE_ROOT_SCHEMA_V4 = "arenyxa.developer-root-trust/v4"
HARDWARE_ROOT_PROTECTION_SCHEMA = "arenyxa.tpm-root-five-layer/v1"
HARDWARE_ROOT_PROTECTION_SCHEMA_V2 = "arenyxa.tpm-root-five-layer/v2"
...
def _public_from_artifact(value: str, algorithm: str = SIGNATURE_ALGORITHM) -> bytes:
    raw = b64u_decode(value, max_bytes=128)
    try:
        return validate_public_key_raw(algorithm, raw)
    except ValueError as exc:
        raise _error(
            "DEVELOPER_PUBLIC_KEY_INVALID",
            "Developer public key does not match its declared signature algorithm",
...
def _verify_signature(
    ...
    public_raw: bytes,
    expected_signer: str,
    signer_algorithm: str = SIGNATURE_ALGORITHM,
) -> None:
    ...
    try:
        expected_algorithm = normalize_signature_algorithm(signer_algorithm)
        block_algorithm = normalize_signature_algorithm(str(block.get("algorithm") or ""))
    except ValueError as exc:
        raise _error("DEVELOPER_SIGNATURE_INVALID", "signature algorithm is unsupported") from exc
    if block_algorithm != expected_algorithm or str(block.get("signer_key_id")) != str(expected_signer):
    ...
    try:
        verify_signature_raw(expected_algorithm, public_raw, signature, canonical_json(_unsigned(artifact)))
    except InvalidSignature as exc:
        raise _error("DEVELOPER_SIGNATURE_INVALID", "developer artifact signature verification failed") from exc


def _validate_hardware_root_profile(value: object) -> dict[str, Any]:
    if not isinstance(value, Mapping):
        raise _error("DEVELOPER_ROOT_INVALID", "Hardware Developer Root protection profile is missing")
    profile = dict(value)
    ...
    if schema == HARDWARE_ROOT_PROTECTION_SCHEMA:
        _exact_fields(
            profile,
            {"schema", "layers", "machine_scope", "private_exportable", "signing_only", "high_protection"},
            "Hardware Developer Root protection profile",
        )
    elif schema == HARDWARE_ROOT_PROTECTION_SCHEMA_V2:
        _exact_fields(
            profile,
            {
                "schema", "layers", "machine_scope", "private_exportable", "signing_only", "high_protection",
                "hardware_required", "provider", "algorithm",
            },
            "Hardware Developer Root protection profile",
        )
        if not bool(profile.get("hardware_required")):
            raise _error("DEVELOPER_ROOT_INVALID", "Hardware Developer Root must require hardware-backed key storage")
        if profile.get("provider") != "windows-tpm-cng" or profile.get("algorithm") != ECDSA_P256_SHA256:
            raise _error("DEVELOPER_ROOT_INVALID", "Hardware Developer Root protection provider/algorithm is invalid")
    else:
        raise _error("DEVELOPER_ROOT_INVALID", "Hardware Developer Root protection profile schema is invalid")
    if profile.get("layers") != expected_layers:
        raise _error("DEVELOPER_ROOT_INVALID", "Hardware Developer Root five-layer protection profile is incomplete")
    if not bool(profile.get("machine_scope")) or bool(profile.get("private_exportable")):
        raise _error("DEVELOPER_ROOT_INVALID", "Hardware Developer Root must be machine-bound and non-exportable")
    if not bool(profile.get("signing_only")) or not bool(profile.get("high_protection")):
        raise _error("DEVELOPER_ROOT_INVALID", "Hardware Developer Root usage/UI protection is incomplete")
    return profile
...
def validate_root_trust(artifact: Mapping[str, Any]) -> dict[str, Any]:
    ...
    elif schema in {HARDWARE_ROOT_SCHEMA, HARDWARE_ROOT_SCHEMA_V3, HARDWARE_ROOT_SCHEMA_V4}:
        fields = {
            "schema", "key_id", "algorithm", "public_key", "fingerprint", "owner_label", "created_at",
            "provider", "key_name", "protection_profile", "signature",
        }
        if schema in {HARDWARE_ROOT_SCHEMA_V3, HARDWARE_ROOT_SCHEMA_V4}:
            fields.update({"generation", "key_binding_sha256"})
        if schema == HARDWARE_ROOT_SCHEMA_V4:
            fields.add("purpose")
        _exact_fields(artifact, fields, "Hardware Developer Root trust")
        algorithm = ECDSA_P256_SHA256
        if artifact.get("algorithm") != algorithm or artifact.get("provider") != "windows-tpm-cng":
            raise _error("DEVELOPER_ROOT_INVALID", "Hardware Developer Root algorithm/provider is invalid")
        key_name = _validate_identifier(str(artifact.get("key_name")), label="hardware root key name")
        if schema == HARDWARE_ROOT_SCHEMA:
            if key_name != "Arenyxa.Developer.HardwareRoot.v1":
                raise _error("DEVELOPER_ROOT_INVALID", "Hardware Developer Root key container name is not the fixed v1 name")
        else:
            ...
            if not isinstance(generation, int) or isinstance(generation, bool) or not 1 <= generation <= 999_999:
                raise _error("DEVELOPER_ROOT_INVALID", "Hardware Developer Root generation is invalid")
            purpose = "primary"
            if schema == HARDWARE_ROOT_SCHEMA_V4:
                purpose = str(artifact.get("purpose") or "").strip().casefold()
                if purpose not in {"primary", "recovery"}:
                    raise _error("DEVELOPER_ROOT_INVALID", "Hardware Developer Root purpose is invalid")
            prefix = (
                "Arenyxa.Developer.HardwareRecoveryRoot.g"
                if purpose == "recovery"
                else "Arenyxa.Developer.HardwareRoot.g"
            )
            expected_name = f"{prefix}{generation:04d}"
            if key_name != expected_name:
                raise _error("DEVELOPER_ROOT_INVALID", "Hardware Developer Root key container does not match its generation")
            binding = str(artifact.get("key_binding_sha256") or "")
            if len(binding) != 64 or any(ch not in "0123456789abcdef" for ch in binding):
                raise _error("DEVELOPER_ROOT_INVALID", "Hardware Developer Root key binding digest is invalid")
        _validate_hardware_root_profile(artifact.get("protection_profile"))
    else:
        raise _error("DEVELOPER_ROOT_INVALID", "Developer Root schema is unsupported")
    key_id = _validate_identifier(str(artifact.get("key_id")), label="root key id")
    public_raw = _public_from_artifact(str(artifact.get("public_key")), algorithm)
    ...
    _verify_signature(artifact, public_raw, key_id, algorithm)
...
def validate_issuer_certificate(
    ...
    *,
    ...
    _verify_signature(
        artifact,
        _public_from_artifact(str(root["public_key"]), str(root.get("algorithm") or SIGNATURE_ALGORITHM)),
        str(root["key_id"]),
        str(root.get("algorithm") or SIGNATURE_ALGORITHM),
    )
    return dict(artifact)
...
def validate_revocation(
    ...
    *,
    ...
    if target_kind == "issuing_key":
        ...
        signer_public = _public_from_artifact(
            str(root["public_key"]), str(root.get("algorithm") or SIGNATURE_ALGORITHM)
        )
        signer_algorithm = str(root.get("algorithm") or SIGNATURE_ALGORITHM)
    else:
        if issuer is None:
        ...
        signer_public = _public_from_artifact(str(issuer["public_key"]), SIGNATURE_ALGORITHM)
        signer_algorithm = SIGNATURE_ALGORITHM
    _verify_signature(artifact, signer_public, signer, signer_algorithm)
    return dict(artifact)
