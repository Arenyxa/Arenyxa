...
class SecurityKernel:
    ...
    def local_foundation(cls, data_root: Path) -> "SecurityKernel":
        configured = os.environ.get("ARENYXA_AUDIT_FAILURE_POLICY", AuditFailurePolicy.FAIL_OPERATIONAL.value).strip().casefold()
        try:
            policy = AuditFailurePolicy(configured)
        except ValueError as exc:
            raise ArenyxaError(
                "AUDIT_POLICY_INVALID",
                "ARENYXA_AUDIT_FAILURE_POLICY must be fail_closed or fail_operational",
                domain="SECURITY",
            ) from exc
        return cls(
            audit=AuditLog(
                Path(data_root) / "security" / "audit.jsonl",
                failure_policy=policy,
                recovery_path=Path(data_root) / "security" / "audit.recovery.jsonl",
            )
        )
    ...
    def authorize(
        ...
        *,
        ...
        platform_root_claimed = (
            session.trust_domain is TrustDomain.DEVELOPER
            and "platform.root" in session.granted_capabilities
        )
        platform_root_integrity = bool(
            isinstance(session.metadata, Mapping)
            and session.metadata.get("root_integrity_verified") is True
        )
        platform_root = bool(platform_root_claimed and platform_root_integrity)
        if definition is None:
            decision = AuthorizationDecision(False, "CAPABILITY_UNKNOWN", "capability is not registered")
        elif platform_root_claimed and not platform_root_integrity:
            decision = AuthorizationDecision(
                False, "ROOT_INTEGRITY_REQUIRED",
                "platform Root authority requires a verified Root integrity projection",
            )
